...
function createVersionsDropdown(filename, files) {
	...
	if (filename) {
		...
		$(html).appendTo($('tr').filterAttr('data-file',filename).find('td.date a.modified'));
	}
	...
	$.ajax({
		...
		success: function( versions ) {
			...
			if (versions) {
				...
				$('#found_versions').hide();
				$('#makelink').hide();
				$('<div style="text-align:center; color:#333;">No other versions available</div>').appendTo('#dropdown-version');
			}
